...
def grow_overlap(startpoint, seq):
    ...
    min_len = minmelt//4 #LA / to //
    # overlap = seq[startpoint - min_len/2 : startpoint] + seq[startpoint : startpoint + min_len/2 ]
    overlap = seq[(startpoint - min_len//2) : startpoint] + seq[startpoint : (startpoint + min_len//2) ] #LA / to //
    counter = 0
    # firsthalf = seq[:(startpoint - min_len/2 - counter)]
    firsthalf = seq[:int(startpoint - min_len//2 - counter)] #LA / to //
    ...
    while (min_len//2 + counter < args.maxoverlaplen) and ( (tm < minmelt) or ( (gc < 40) or (gc > 60) ) ): # GC% must be between 40 and 60, Tm should be above mininimum, and the overlap should not be longer than 30bps
        ...
        overlap = seq[(startpoint - min_len//2 - counter)] + overlap + seq[(startpoint + min_len//2 + counter - 1)] ##LA / to //
        tm = MeltingTemp.Tm_NN(overlap, Na=50, K=0, Tris=0, Mg=args.MgmM, dNTPs=args.dNTPsmM )
        gc = GC(overlap)
    #print(counter)
    firsthalf = seq[:(startpoint - min_len//2 - counter)] #LA / to //
    secondhalf = seq[(startpoint + min_len//2 + counter):len(seq)] #LA / to //
...
for line in lines:
    ...
    if len(its) != 3:
        ...
        #LA: change to fit Longxing's file output, input format name\tAA_seq\tNucleotide_seq
...
for design in designs:
    ...
    # startseq = len(seq)/2
    startseq = int(len(seq)/2) # LA changed to make int
    # Checks:
    ...
    if ((current_len_5prime_oligo > max_oligo_size ) or (current_len_3prime_oligo > max_oligo_size )) or (not ( (tm>= minmelt ) and ( (gc>40) and (gc<60) ) and (overlapsegment_len<= args.maxoverlaplen) ) ):
        ...
        print(design, seq,file = impossible_sequences)
    ...
    if (len(missaligned1) > 1) and not skip:
        ...
        if fixed:
            ...
            print("%s,%s,%d,%02f,%d"%(design,overlapsegment,len(overlapsegment),Tm,GC_cont),file = overlap_report_handle)

            print(design , seq, file = final_sequences)
            names.append(design)
            ...
            trashed_seq[design] = seq
            print(design, seq, file = trashed_sequences)
...
for idx in range(len(oligos3)):
    ...
    if fivePlen < upper_limit and threePlen < upper_limit :
        ...
        print(names[idx] + '_1st' , s1, file = final_order_large)
        print(names[idx] + '_2nd' , s2, file = final_order_large)
    ...
    print(names[idx], "is too big!!" , fivePlen, threePlen, "limits:" , lower_limit, upper_limit, oligos5[idx], oligos3[idx],file=report)
...
print("adapter distributions:", distributions, file = report)
